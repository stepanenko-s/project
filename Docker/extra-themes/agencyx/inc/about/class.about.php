...
if (!class_exists('agencyx_About')) {
	...
	class agencyx_About
	{
		 ...
		 *
		 * @var agencyx_About $instance The  TI_About_Page instance.
		 */
		...
		{
			if (!isset(self::$instance) && !(self::$instance instanceof agencyx_About)) {
				self::$instance = new agencyx_About;
		...
		public function ti_about_page_render()
		{
			...
			if (!empty($welcome_title) || !empty($welcome_content) || !empty($this->tabs)) {
				...
				echo '<div class="wrap about-wrap agencyx-about-wrap">';
				...
				echo '<a href="' . esc_url('https://wpthemespace.com/product/agencyx-pro/') . '" target="_blank" class="wp-badge epsilon-welcome-logo"></a>';
...
?>
			<div class="feature-section agencyx-section agencyx-section-getting-started three-col">
				<?php foreach ($content as $item) : ?>
					<?php $this->render_grid_item($item); ?>
				<?php endforeach; ?>
			</div><!-- .feature-section .agencyx-section -->
		<?php
		}
		...
		?>
			<div class="feature-section agencyx-section agencyx-section-support three-col">
		...
		public function free_pro()
		{
			...
			if (!empty($free_pro)) {
				if (!empty($free_pro['free_theme_name']) && !empty($free_pro['pro_theme_name']) && !empty($free_pro['features']) && is_array($free_pro['features'])) {
					...
					echo '<tr><td style="border-top:0;"></td><td style="border-top:0;"></td><td style="border-top:0;" class="only-pro"><a href="' . esc_url('https://wpthemespace.com/product/agencyx-pro/') . '" target="_blank" >';
					echo esc_html('AgencyX Pro Live Preview', 'agencyx');
					echo '</a></td></tr>';
			...
			if ('appearance_page_' . $this->theme_slug . '-welcome' == $hook_suffix) {
				...
				wp_localize_script(
					...
					array(
						...
						'activating_string'        => __('Activating', 'agencyx')
					)
				);
			}
		}
